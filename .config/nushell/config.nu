...
$env.PATH = ($env.PATH | split row (char esep) | append "/home/axlrnzo/.local/bin")
$env.PATH = ($env.PATH | split row (char esep) | append "/home/axlrnzo/zig-linux-x86_64-0.14.0-dev.1472+3929cac15")
...
$env.NIX_PATH = $"($env.HOME)/.nix-defexpr/channels"
...
def --env load-nix-profile [] {
    let profile = $"($env.HOME)/.nix-profile/etc/profile.d/nix.sh"
    if ($profile | path exists) {
        let nix_env = (bash -c $"source ($profile) && env" | lines | parse "{name}={value}")
        for item in $nix_env {
